...
$tab_id = $_GET['tab_id'] ?? $_POST['tab_id'] ?? 'default';

// Debug: Log session with tab info
error_log('TAB SPECIFIC USER INFO - Tab ID: ' . $tab_id . ' - Session: ' . print_r($_SESSION, true));
...
try {
    ...
    switch ($user_type) {
        ...
        case 'cso':
            ...
            $stmt = $conn->prepare("SELECT cso_id, email, full_name, role, profile_photo FROM csos WHERE cso_id = ? AND email = ?");
            $stmt->execute([$user_id, $_SESSION['email']]);
    ...
    if ($user_data) {
        ...
        // Log successful identification with tab info
        error_log('User identified in tab ' . $tab_id . ': ' . $user_data['user_type'] . ' - ' . $user_data['full_name'] . ' via ' . $user_data['auth_method']);
    ...
    error_log('Database error in tab specific user info: ' . $e->getMessage());

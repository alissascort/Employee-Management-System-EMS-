...
/**
 * enable_api.php
 * Sets is_active = 1 for a given API endpoint.
 * CSO-only.
 */
...
try {
    ...
    $stmt = $db->prepare('UPDATE api_endpoints SET is_active = 1, status = \'up\' WHERE id = ?');
    $stmt->execute([$id]);
    ...
    error_log('enable_api error: ' . $e->getMessage());
